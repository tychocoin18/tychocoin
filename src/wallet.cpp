...
// tychocoin: optional setting to unlock wallet for block minting only;
...
// tychocoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // tychocoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // tychocoin: sub-cent change is moved to fee
...
// tychocoin: create coin stake transaction
...
// tychocoin: check 'spent' consistency between wallet and txindex
// tychocoin: fix wallet spent state according to txindex
...
// tychocoin: disable transaction (only for coinstake)
